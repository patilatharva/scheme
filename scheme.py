...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        macro = env.lookup(first)
        return scheme_eval(macro.apply_macro(rest, env), env)
    else:
        # BEGIN PROBLEM 4
        procedure = scheme_eval(expr.first, env)
        check_procedure(procedure)
        args = expr.rest.map(lambda el: scheme_eval(el, env))
        return scheme_apply(procedure, args, env)
        # END PROBLEM 4

"""
macro.body = (list (quote map) (list (quote lambda) (list formal) body) iterable)
macro.formals = (formal iterable body)
rest = (i (quote (1 2 3)) (if (= i 1) 0 i))
macro = (lambda (formal iterable body) list (quote map) (list (quote lambda) (list formal) body) iterable)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('too few or too many values.')
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    lambda_form = LambdaProcedure(formals, expressions.rest, env)
    return lambda_form
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions is nil:
        return scheme_truep(expressions)
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions is nil:
        return not scheme_truep(expressions)
...
def do_define_macro(expressions, env):
    ...
    identifier = expressions.first.first

    formals = expressions.first.rest
    check_formals(formals)
    body = expressions.rest
    
    macro_form = MacroProcedure(formals, body, env)
    env.define(identifier, macro_form)
    return identifier
    # END Problem 20
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 15
    mu_form = MuProcedure(formals, expressions.rest)
    return mu_form
    # END PROBLEM 15
